...
## 2026-07-25 - [Missing Retry-After Header on Express Rate Limiting Responses]
**Vulnerability:** When the Express rate limiter middleware rejected requests exceeding the threshold (`HTTP 429 Too Many Requests`), it did not return a `Retry-After` HTTP header. This violated RFC 6585 and left clients without back-off guidance, potentially leading to immediate/rapid retry loops and thundering herd conditions during high server load (CWE-770).
**Learning:** Rate-limiting error responses must inform clients of the remaining window duration so well-behaved clients and automated systems can pause requests until reset time.
**Prevention:** Always set `res.setHeader('Retry-After', String(Math.ceil((resetTime - now) / 1000)))` when issuing HTTP 429 status code responses.
