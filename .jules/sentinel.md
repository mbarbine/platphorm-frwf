...
## 2026-07-23 - [Unhandled Exception and Stack Trace Information Disclosure in MCP and Route Compliance Serverless Handlers]
**Vulnerability:** The Vercel serverless functions `api/mcp.js` and `api/v1/route-compliance.js` lacked outer try-catch blocks. Any unhandled server runtime or parsing exception could crash the serverless container execution context and leak internal stack trace context, directory setups, or environmental details to client users (CWE-209).
**Learning:** Unhandled exceptions in backend handlers are hazardous if allowed to bubble up to default hosting environments, which often return verbose error logs or system details. Encapsulating handlers in generic try-catch blocks ensures secure-by-default behavior under any runtime error.
**Prevention:** Wrap entry execution pathways of JS/TS serverless handlers in outer try-catch blocks, log errors via `globalThis.console.error`, and return clean generic error statuses.
