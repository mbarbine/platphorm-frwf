...
## 2026-07-24 - [Unvalidated Host Extraction from Chained X-Forwarded-Host Headers]
**Vulnerability:** The route compliance redirect handler in `api/v1/route-compliance.js` extracted the target host by calling `split(":")[0]`. When requests traversed multiple proxies resulting in a comma-separated `X-Forwarded-Host` chain (e.g. `untrusted.com, trusted.platphormnews.com`), splitting only on `:` allowed untrusted/malicious client-supplied headers to bypass origin verification or cause improper domain redirection (CWE-290 / CWE-346).
**Learning:** `X-Forwarded-Host` header values can be comma-separated lists when populated across multiple proxy hops. Extracting domain names without first isolating the initial hop (`.split(",")[0].trim()`) leads to insecure host matching.
**Prevention:** Always split `X-Forwarded-Host` header values by `,` first to isolate the initial client-facing hop before stripping port numbers or matching against trusted domain whitelists.
