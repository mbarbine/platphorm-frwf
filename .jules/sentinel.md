...
## 2026-08-20 - [Domain Whitelist Bypass via Chained X-Forwarded-Host Headers]
**Vulnerability:** In `api/v1/route-compliance.js`, the domain verification logic called `.split(":")[0]` directly on the raw `X-Forwarded-Host` header. When requests were forwarded through multiple proxies creating a comma-delimited list of host headers (e.g. `attacker.com, platphormnews.com`), `trustedSite` received the entire unsplit string, causing host validation to fail or be spoofed (CWE-290 / CWE-346).
**Learning:** `X-Forwarded-Host` headers set by multi-tiered reverse proxies or edge networks can contain comma-separated entries where the client's original host appears first. Validating host headers without extracting the first element allows header-injection or domain bypass vectors.
**Prevention:** Always extract the first host entry from comma-delimited proxy headers (`.split(',')[0].trim()`) before stripping port numbers and running hostname whitelist checks.
