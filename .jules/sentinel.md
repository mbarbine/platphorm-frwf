...
## 2026-07-25 - [Strict HTTP Method Restriction on Vercel Serverless Redirect Handler]
**Vulnerability:** The Vercel serverless redirect route `api/v1/route-compliance.js` accepted any incoming HTTP request method (e.g. `POST`, `PUT`, `DELETE`). Redirecting non-safe HTTP methods using a 307 redirect preserves the HTTP verb and payload, presenting downstream parameter override, injection, or unauthorized side effects.
**Learning:** Redirect endpoints designed only to process resource locator routing must strictly enforce safe read-only methods (`GET` and `HEAD`). Additionally, unit test suites mocking incoming request structures should explicitly specify the HTTP `method` parameter to avoid unhandled property access errors.
**Prevention:** Add a strict check at the entry point of the serverless function handler to verify if `request.method` is `GET` or `HEAD`. If not, respond with a `405 Method Not Allowed` status code along with the `Allow` header.
