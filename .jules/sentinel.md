...
## 2026-07-23 - [Unhandled Exception and Stack Trace Leak Mitigation in MCP Endpoint]
**Vulnerability:** Unhandled runtime exceptions in `api/mcp.js` could crash the serverless container execution context and disclose internal stack traces or environment details (CWE-209).
**Learning:** Top-level handlers in serverless functions must defensively catch unexpected exceptions and log them internally to prevent leaking internal callstacks or execution environments.
**Prevention:** Wrap serverless request handlers in `try...catch` blocks, log the exception via `globalThis.console.error`, and return a sanitized standard error response (e.g. JSON-RPC error `-32603: Internal error`) with a 500 status code.
