...
## 2026-07-24 - [Unsanitized Chained Proxy Host Headers in Route Compliance Handler]
**Vulnerability:** In `api/v1/route-compliance.js`, the `X-Forwarded-Host` header was split by colon (`:`) directly without first splitting by comma (`,`). When requests passed through multiple reverse proxies or load balancers sending chained host headers (e.g. `platphormnews.com:443, proxy2.domain.com`), the host string retained trailing proxy hostnames, causing domain verification to fail or host spoofing risks (CWE-290 / CWE-346).
**Learning:** Multi-hop proxy setups concatenate client-facing and proxy-facing host information into `X-Forwarded-Host` separated by commas. Extracting only the first element ensures the original client request domain is validated.
**Prevention:** Always split `X-Forwarded-Host` by comma first (`.split(',')[0].trim()`) before stripping port numbers (`.split(':')[0]`).
