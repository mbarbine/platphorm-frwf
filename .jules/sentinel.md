...
## 2026-07-23 - [Information Disclosure and Technology Leakage via Unhandled Serverless Exceptions]
**Vulnerability:** The Vercel serverless functions `api/mcp.js` and `api/v1/route-compliance.js` were lacking a top-level try-catch error handler. Any unhandled TypeError, SyntaxError, or internal exception (e.g. from missing request headers or parsing malformed payloads) would crash the serverless container and leak verbose environment information, framework/hosting details, and internal folder path structures to the client (CWE-209).
**Learning:** Modern serverless hosting platforms default to verbose error outputs during unexpected runtime failures. Without centralized defensive try-catch wrappers around the main execution path of every serverless route, minor inputs can cause disclosure of system internals.
**Prevention:** Always wrap the entire body of serverless route handlers in a secure try-catch block. Log the detailed error trace internally to standard error streams, and return a clean, generic, and sanitized JSON response to the client.

## 2026-07-24 - [Chained Host Header Domain Verification Bypass]
**Vulnerability:** The `api/v1/route-compliance.js` endpoint extracted the `x-forwarded-host` header and split it directly by colon (`:`). When an attacker supplied a comma-separated list of hosts (e.g., `evil.com, sub.platphormnews.com`), `String(header).split(":")[0]` resulted in `evil.com, sub.platphormnews.com`, which passed `endsWith(".platphormnews.com")`, bypassing host domain validation (CWE-290 / CWE-346).
**Learning:** `X-Forwarded-Host` headers added by forward proxies/gateways can be comma-separated strings representing client-to-proxy chains. Splitting by colon before isolating the first comma-separated entry can lead to validation bypasses if string matching logic checks domain suffixes across the concatenated string.
**Prevention:** Always extract the first IP/domain entry using `.split(',')[0].trim()` from `X-Forwarded-Host` before stripping port numbers or verifying origin domain whitelists.

## 2026-07-25 - [Missing Retry-After Header on Rate Limited Responses]
**Vulnerability:** Express endpoints returning HTTP 429 status during rate limiting lacked the RFC 6585 compliant `Retry-After` header. Without explicit backoff signals, clients and automated scripts are prone to immediate rapid retries, worsening server load and potentially causing thundering herd DoS conditions.
**Learning:** Rate-limiting implementations must communicate retry backoff periods explicitly via HTTP headers to guide client behavior and maintain standards compliance.
**Prevention:** Always calculate seconds remaining until rate limit window reset (`Math.ceil((resetTime - now) / 1000)`) and set `res.setHeader('Retry-After', retryAfterSeconds)` when responding with HTTP 429 status.
