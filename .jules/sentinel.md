...
## 2026-07-24 - [Domain Whitelist Bypass via Multi-Host X-Forwarded-Host Header]
**Vulnerability:** The route compliance redirect handler `api/v1/route-compliance.js` evaluated `X-Forwarded-Host` without splitting comma-separated proxy header lists. An attacker sending a chained header like `X-Forwarded-Host: evil.com, platphormnews.com` passed the `.endsWith(".platphormnews.com")` domain check, bypassing domain whitelist validation (CWE-290 / CWE-346).
**Learning:** Standard HTTP proxy forwarding headers like `X-Forwarded-Host` can contain multiple comma-separated domain values appended by downstream proxies. Parsing only by colon (`.split(":")[0]`) leaves trailing domains intact in the string, causing trailing domain pattern checks to falsely match untrusted hosts.
**Prevention:** Always extract and sanitize the first host entry from comma-separated forwarding headers (`.split(",")[0].trim()`) before applying domain whitelist checks or port stripping.
