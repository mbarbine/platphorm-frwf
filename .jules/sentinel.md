...
## 2026-07-23 - [Unhandled Exception Disclosure and Crash Prevention in Vercel Serverless Handlers]
**Vulnerability:** Unhandled runtime exceptions in Vercel serverless handlers (`api/mcp.js` and `api/v1/route-compliance.js`) were not intercepted, which could crash the serverless container or disclose internal stack traces, environmental setups, or source code context to end-users (CWE-209).
**Learning:** Serverless execution runtimes default to verbose HTML error outputs when exceptions propagate past the main entry function. Explicit try-catch boundaries must wrap serverless endpoints to catch unexpected errors, log them securely server-side, and return a clean, sanitized HTTP 500 error.
**Prevention:** Always wrap the entire body of serverless handler entry points in standard try-catch blocks and log internal errors via `globalThis.console.error` to avoid leaking diagnostic data or system telemetry on failures.
