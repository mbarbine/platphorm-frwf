...
## 2026-07-25 - [Optimize Math.hypot in High-Frequency Animation and Orientation Systems]
**Learning:** Calling `Math.hypot` inside animation representation or pose resolution helpers (`locomotionPresentation.ts` and `combatOrientation.ts`) that are processed within React Three Fiber rendering loops introduces massive CPU overhead due to internal overflow/underflow checks. Since coordinates in this game space are well-bounded, standard `Math.sqrt` of squared coordinate sums operates up to ~8x faster.
**Action:** Avoid utilizing `Math.hypot` inside high-frequency animation and tracking callbacks; use manual squared-sum `Math.sqrt` calculations or direct squared comparison logic to preserve smooth frame delivery.
