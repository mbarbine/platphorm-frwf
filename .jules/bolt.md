...
## 2025-02-28 - [Avoided GC Churn and Square Roots in High-Frequency Segment Validation Checks]
**Learning:** Checking body segments via `inspectNumericalBody` happens on every active joint/part of all fighters during every fixed physics tick (thousands of times per second). Constructing inline array helpers like `finite([...])` for `Number.isFinite` tests creates immense GC pressure from continuous array instantiations. In addition, computing velocities directly via `Math.hypot` is highly redundant when limits are healthy.
**Action:** Replace array-based validations with inline individual field checks, and use squared-magnitude thresholds (`linearSpeedSq > 576` instead of `Math.hypot > 24`) to completely bypass square roots on normal path executions, calling `Math.sqrt` only to record the exact runaway value.
