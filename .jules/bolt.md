...
## 2025-03-01 - [Optimized Directional and Context Resolver Math.hypot Calls]
**Learning:** Performing `Math.hypot` inside extremely hot execution paths such as directional calculations (`combatDirection`) and action context checks (`resolveContextAction` and `resolvePropAction`) is completely unnecessary. Bounded 2D controller axes or local wrestling arena coordinates do not suffer from overflow or underflow. Using zero-allocation squared-magnitude comparisons instead of `Math.hypot` bypasses both the underflow checks and the square root calculation itself.
**Action:** Replace `Math.hypot` inside control direction mapping and contextual logic solvers with flat, squared-magnitude comparisons (e.g. against `0.1225` and `9.9225`) to prevent hot path overhead.
