...
## 2026-07-25 - [Optimized Math.hypot with Math.sqrt and Squared-Magnitude Comparisons in Online Authority Match Simulation]
**Learning:** In the authoritative multiplayer tick simulation (`onlineSimulation.ts`), calculating distance/swept contacts via `Math.hypot` is a severe performance bottleneck. Because the ring coordinate space is small and bounded, standard `Math.sqrt` is perfectly safe and ~8x faster. Furthermore, refactoring high-frequency checks (e.g. swept distance limits in `segmentCircleHit`, proximity checks, and movement threshold evaluations in `stepOnlineMatch`) to flat, zero-allocation squared-magnitude checks avoids square root extraction completely.
**Action:** Replace local `length` helpers with `Math.sqrt(x * x + z * z)` and refactor hot-path distance checks into squared comparisons against pre-squared thresholds.
