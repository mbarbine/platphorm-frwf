...
## 2026-07-25 - [Optimized Math.hypot in Presentation, Orientation, and Camera Director Loops]
**Learning:** In a high-frequency animation and presentation context (e.g., evaluating combat orientation, classifying locomotion, or running camera shot selection), standard 2D Math.hypot calls introduce unnecessary dynamic scaling and lookup overhead. Computing standard Math.sqrt for 2D vectors is up to 8x faster in modern JS engines and yields significant CPU time savings in hot frame loops.
**Action:** Replace 2D Math.hypot calls with direct Math.sqrt calculations inside resolution and classification helpers (e.g., locomotionPresentation.ts, combatOrientation.ts, and cameraDirector.ts).
