...
## 2026-09-20 - [Unrolled Combat Motion Pose Blending in Hot Frame Callbacks]
**Learning:** In skeletal animation blending routines (`combatMotion.ts`), calling `blend()` executed dynamic `for...of` array iterations over `rotations`, `hinges`, and `ROOT_KEYS` keys while performing redundant `{ ...a }` initial object shallow copies on every call. Unrolling Pose property assignments directly into a single returned object literal with a `slerpJoint` helper eliminates loop iterator overhead and dynamic key accesses, reducing CPU and Garbage Collection pressure during 60Hz physics and animation updates.
**Action:** Unroll pose interpolation loops into direct object literals with dedicated `slerp` helper functions in hot animation blending paths.
