...
## 2026-07-25 - [Optimized Online Multiplayer Simulation via Math.sqrt and Squared Comparisons]
**Learning:** In highly nested, high-frequency online match tick loops (such as the online multiplayer simulation step), using `Math.hypot` to compute vector lengths or distances introduces substantial floating-point scaling overhead. Bounded wrestling coordinates are completely safe from overflow, making standard `Math.sqrt` and zero-allocation squared comparisons (`dx * dx + dz * dz < limit_squared`) up to 8x faster and completely bypassing the square root calculation in distance/proximity checks.
**Action:** Replace `Math.hypot` with standard `Math.sqrt` in core mathematical/length helper functions, and replace active length/distance checks in the simulation's tick and collision solvers with squared-magnitude bounds comparison.
