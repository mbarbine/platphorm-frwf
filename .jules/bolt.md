...
## 2026-07-24 - [Optimized Multiplayer Online Simulation Mathematics]
**Learning:** In multiplayer online simulations (`onlineSimulation.ts`), calculating distance or evaluating swept active contact using `Math.hypot` places significant CPU overhead on high-frequency execution paths. Standard `Math.sqrt` is up to 8x faster and perfectly safe for bounded 2D game configurations. Furthermore, utilizing zero-allocation squared-magnitude comparisons (`diffX * diffX + diffZ * diffZ <= radius * radius`) completely bypasses square-root computation overhead.
**Action:** Replace `Math.hypot` with standard `Math.sqrt` for vector length computations, and refactor proximity, locomotion, and swept contact checks to use zero-allocation squared comparisons.
