...
## 2026-07-25 - [Optimized Online Multiplayer Simulation via Squared-Magnitude Checks]
**Learning:** High-frequency multiplayer online simulation checks like swept active contact validations (`segmentCircleHit`), player distance checks, and locomotion state thresholds inside `onlineSimulation.ts` degrade performance when using standard length metrics. Replacing array-based or `Math.hypot`/`Math.sqrt` operations with zero-allocation flat squared-magnitude comparisons (`diffX * diffX + diffZ * diffZ <= radius * radius`) completely bypasses the costly square root extraction in the hot path.
**Action:** Always utilize flat squared-magnitude calculations and pre-squared constants for proximity, collision, and threshold checks inside multiplayer online step loops.
