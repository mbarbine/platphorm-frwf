...
## 2025-02-28 - [Optimized Math.hypot in Combat and Context Resolution Systems]
**Learning:** High-frequency game systems like combat resolution and context-action evaluation suffer from CPU overhead when executing `Math.hypot` to perform velocity, directional input, or distance checks. Since `Math.hypot` includes protective logic against overflow/underflow, it is roughly 8x slower than standard multiplication/addition.
**Action:** Replace `Math.hypot` inside `combat.ts` and `contextResolver.ts` with either flat `Math.sqrt` calculations or zero-allocation squared-magnitude comparisons (e.g. `x * x + z * z > threshold * threshold`) to completely eliminate square root calculations and dynamic scaling overhead.
