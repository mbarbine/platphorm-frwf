...
## 2026-07-25 - [Optimized Math.hypot with Math.sqrt and Squared-Magnitude Checks in Online Multiplayer Simulation]
**Learning:** In multiplayer online simulation checks (`packages/game-core/src/onlineSimulation.ts`), replacing standard `Math.hypot` with standard `Math.sqrt` avoids dynamic scaling overhead. Furthermore, refactoring hot contact path checks (like `segmentCircleHit`), target proximity, and locomotion checks to use zero-allocation squared-magnitude comparisons completely eliminates costly square-root calculations, providing significant performance gains in deterministic simulations.
**Action:** Avoid calling `Math.hypot` in online deterministic step loops, and use squared-magnitude comparisons for proximity, locomotion, and collision checks.
