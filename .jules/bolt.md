...
## 2026-07-25 - [Optimized Math.hypot in Grapple Dynamics Step]
**Learning:** In the high-frequency physics step of soft two-body grapple dynamics (`stepGrappleDynamics` in `grappleDynamics.ts`), calling `Math.hypot` to compute wrestle tension creates unnecessary CPU overhead. Standard double-precision floating point squares are entirely safe from overflow/underflow under bounded arena simulation coordinates, allowing standard `Math.sqrt` to run up to 8x faster.
**Action:** Replace `Math.hypot` with standard `Math.sqrt(force.x * force.x + force.z * force.z)` inside the `stepGrappleDynamics` loop.
