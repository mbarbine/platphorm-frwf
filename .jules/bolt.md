...
## 2026-07-25 - [Optimized Math.hypot with Math.sqrt and Distance Squared Reuse in Arena Mat Render Loop]
**Learning:** In the high-frequency React Three Fiber render loop for the `ReactiveMat` grid (evaluating 108 mesh tiles on every frame), using `Math.hypot` to calculate distance from the epicenter of an impact introduces massive CPU overhead due to its dynamic scaling logic. Replacing it with standard `Math.sqrt` and reusing the squared distance (`distanceSq`) for exponential decay calculations saves hundreds of slow operations per frame without affecting visual fidelity.
**Action:** Replace `Math.hypot` with standard `Math.sqrt` and reuse squared distance `distanceSq` inside the `ReactiveMat` and `Post` useFrame loops in `Arena.tsx` to keep the crowd and mat animations highly efficient.
