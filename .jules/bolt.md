...
## 2026-07-25 - [Optimized inspectNumericalBody Validation Loops]
**Learning:** Using array-based iteration helpers like `[v1, v2, ...].every(Number.isFinite)` inside high-frequency verification checks creates garbage collection pressure because it allocates arrays on every invocation. Additionally, executing `Math.hypot` to compute 3D speeds for constraint checks introduces unnecessary CPU overhead when compared directly to pre-squared constant thresholds.
**Action:** Perform inline `!Number.isFinite` evaluations instead of allocating a checklist array. Replace linear and angular speed `Math.hypot` computations with flat squared-sum checks against the pre-computed squares of maximum velocity limits (e.g., `576` for `24` m/s and `1024` for `32` rad/s).
