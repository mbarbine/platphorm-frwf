...
## 2026-07-24 - [Avoid Array Allocation and Math.hypot in High-Frequency inspectNumericalBody Check]
**Learning:** Using an array wrapper and `.every()` inside high-frequency checks like `inspectNumericalBody` generates continuous GC allocations. Furthermore, utilizing `Math.hypot` for speed magnitude checks is costly due to overflow/underflow protection scaling overhead.
**Action:** Replaced the array finite check helper with inline flat checks, and replaced linear/angular speed `Math.hypot` checks with zero-allocation squared-magnitude comparisons against pre-squared constants (`576` and `1024`). Math.sqrt is now only computed as a fallback if a runaway state is actually hit, resulting in massive speedups in the normal simulation path.
