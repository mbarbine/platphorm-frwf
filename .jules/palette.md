...
## 2025-02-22 - Interactive Timers Pause-on-Interaction (Hover and Focus)
**Learning:** Rigid timers that automatically close informative overlays (such as game control tutorials) can cause severe accessibility and usability issues for users requiring extra reading or parsing time, violating basic timing control guidelines (WCAG 2.2.1). Allowing users to temporarily pause the countdown simply by hovering the mouse pointer or focusing elements inside the overlay with a keyboard or screen reader provides a highly intuitive and low-friction timing adjustment experience.
**Action:** Always implement hover (`onMouseEnter`/`onMouseLeave`) and focus (`onFocus`/`onBlur`) event listener hooks on auto-dismissing overlays to conditionally freeze active countdown timers.
