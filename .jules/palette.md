...
## 2025-02-22 - Fighter Select Keyboard Navigation and Live Screen-Reader Announcements
**Learning:** Selection-based menus (such as character roster grids) often lack natural keyboard accessibility and require custom handling for arrow keys (Up, Down, Left, Right) with wrap-around behavior to feel fluent for keyboard-only users. Programmatic focus management must instantly transfer focus to the active option, while a visually hidden aria-live polite region provides immediate spoken context of the selected entity's details (such as name and archetype) to non-visual users.
**Action:** For selection-based lists/grids, add custom onKeyDown handlers with arrow-key wrapping, programmatic focus shifting via targeted ID/data attributes, and a dedicated visually hidden aria-live wrapper to statefully track selection.
