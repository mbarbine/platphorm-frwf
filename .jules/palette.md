...
## 2026-07-22 - Accessible Keyboard Navigation on Selection Grids and Live Announcements
**Learning:** Selection grids (like the Fighter Select roster) that are easily interactive with pointing devices can be frustrating or completely unnavigable for screen reader or keyboard-only users without standard keyboard navigation support. Adding sequential `ArrowUp`/`ArrowDown`/`ArrowLeft`/`ArrowRight` event mapping, focusing the active button element programmatically via custom `data-fighter-select-id` triggers, and coupling it with a visually hidden `aria-live="polite"` element allows assistive technologies to announce selections immediately and seamlessly.
**Action:** Use arrow key listeners to control active index selection, programmatically update DOM focus to matching button references, and provide screen readers with a visually hidden live announcer region.
