...
## 2025-02-22 - Wrap-Around Arrow Navigation and Dynamic Screen Reader Announcements
**Learning:** For roster or grid-based selection screens (such as Fighter Select), standard tab-indexing can feel tedious and exhausting for keyboard-only users. Implementing ArrowUp, ArrowDown, ArrowLeft, and ArrowRight handlers with wrap-around boundaries on roster buttons significantly lowers user interaction friction. Furthermore, adding a visually hidden `aria-live="polite"` feedback region that dynamically announces the selected wrestler's name, archetype, and nickname ensures that screen-reader users receive prompt and clear cues as they browse.
**Action:** Implement wrap-around arrow-key listeners on roster button groups and pair with a visually hidden aria-live notification node to dynamically announce state shifts.
