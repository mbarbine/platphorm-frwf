...
## 2025-02-22 - Wrap-Around Keyboard Navigation and Live Region Support for Fighter Selection
**Learning:** Interactive lists/rosters of choices (such as the Fighter Select screen) can be tedious to navigate with keyboard only if users have to tab through every single card. Adding structured arrow-key navigation (ArrowUp/Down/Left/Right) with seamless wrap-around and immediate focus transfer dramatically speeds up selection for power users and users with motor impairments. Coupling this with a visually hidden `aria-live="polite"` announcement ensures screen-reader users are immediately notified of the selected fighter and their archetype.
**Action:** Implement keyboard event listeners on roster selection grids to manage arrow-key focus transfer, and back each selection with a visually hidden polite screen reader live region.
