...
## 2025-02-24 - Accessible Roster Selection and Wrap-Around Keyboard Navigation
**Learning:** Interactive lists or grids such as character select screens can be highly frustrating for keyboard-only and screen-reader users if selection requires mouse interaction or custom tabs, and changes are not announced dynamically. Implementing keyboard-driven arrow-key (Up/Down/Left/Right) selection with automatic wrapping and dynamic visual focus shifting via custom target attributes, coupled with a visually-hidden politely live wrapper, ensures fully responsive, accessible select screens.
**Action:** Use programmatic Arrow key listeners to update selections with wrap-around boundaries, shift programmatic `.focus()` cleanly with unique DOM attributes, and always announce selected options with inline visually-hidden polite aria-live tags.
