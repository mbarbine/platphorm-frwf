...
## 2025-02-22 - Wrap-Around Keyboard Grid Navigation and Active Selection ARIA Alerts
**Learning:** Large, selection-based layouts like a fighter select roster can be tedious or impossible to navigate for keyboard-only or screen-reader users if they have to tab through every single card or navigate with no verbal confirmation. Implementing custom keyboard event handlers with wrap-around ArrowUp/Down/Left/Right logic, programmatically transferring focus using data attributes, and announcing selection state changes dynamically using a visually hidden `aria-live="polite"` region significantly enhances interactive efficiency and a11y.
**Action:** Build Arrow key keydown handling with wrap-around boundaries, focus targeting, and live announcer elements for roster and option grid selectors.
