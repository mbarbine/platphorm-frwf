...
## 2025-02-22 - Dynamic Keyboard Roster Navigation and Screen Reader Synchronization
**Learning:** Grid-like character selection rosters without keyboard arrow key support force users to tab through every single element, which is tedious and inaccessible. Implementing dynamic wrap-around keyboard navigation (using ArrowUp/Down/Left/Right) improves physical playability. To ensure screen-reader parity, a visually hidden `aria-live="polite"` live region must announce the newly selected option on focus/arrow-change events, providing real-time parity without cluttered visual elements.
**Action:** Use programmatic focus management (`data-fighter-select-id` or similar) paired with a hidden polite screen-reader announcement wrapper during custom keyboard-driven roster navigation.
