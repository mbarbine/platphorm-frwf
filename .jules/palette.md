...
## 2025-02-22 - Timed Tutorial Overlay Adjustable Duration support
**Learning:** Automatically disappearing content or notification overlays (like the 13-second timed tutorial) can cause a major accessibility barrier for users who need more time to read or interact with the text. Implementing a mouse-hover (`onMouseEnter`/`onMouseLeave`) and keyboard-focus (`onFocus`/`onBlur`) pause-and-resume timer mechanism directly on the overlay satisfies WCAG 2.2 Success Criterion 2.2.1 (Timing Adjustable). This allows users to hold pointer or keyboard focus to suspend the automatic closing action indefinitely.
**Action:** Standardize timed tutorial overlays or auto-dismissing notifications to pause timing whenever they are hovered or focused, and gracefully calculate remaining duration when they resume.
