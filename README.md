...
Five original fighters compete in **FRWF Arena** in Singles and five-wrestler Battle Royale. Combat rules are deterministic and phase-based. A 16-body articulated Rapier contact rig per fighter owns locomotion, hand contact, two-hand grips, lift weight, knockback, and measured landings, while a richer hierarchical wrestler model mirrors that authoritative state to keep the player-facing silhouette stable and expressive.
...
L enters a visible collar-and-elbow lock and starts the neutral Voltage Slam/body-slam path by default; a follow-up direction plus J/K/L selects one of fifteen named outcomes including arm drags, trips, chokes, spinebusters, suplexes, powerbombs, side tosses, slams, the preserved neutral Voltage Piledriver, and an Irish whip. Near a corner, F converts that secured clinch into a physical Turnbuckle Rail Shot. A direction held through release also drives the physical throw. When the defender is already committed over the commentary desk, bounded environmental aim assist keeps the slam on the desk instead of launching the ragdoll past it. Hold I/LT to guard: strikes deal tiny chip damage and drain guard stamina, while a grapple can be stuffed until the guard breaks. Space performs a visible, stamina-bound Livewire Kick-Up while downed; ordinary recovery selects a readable back, front, or side get-up from the last impact.

The live control deck can be set to Full, Compact, Prompts Only, or Hidden. Full shows every binding, Compact keeps the five primary actions, Prompts Only retains resolver/action feedback without the deck, and Hidden removes coaching overlays while leaving gameplay input active. Move, context, and prop labels come from the same selection/resolution state used when the action executes.
