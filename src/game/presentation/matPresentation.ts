...
export const authoredDeckPoseOwnsRoot = (fighter: FighterRuntime | null | undefined): boolean => Boolean(fighter && DECK_BOUND_STATES.has(fighter.state));
...
export const visiblePelvisDrop = (fighter: FighterRuntime | null | undefined): number => {
  if (!fighter || authoredDeckPoseOwnsRoot(fighter)) return 0;
