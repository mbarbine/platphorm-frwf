...
export class BodyWorksRuntime {
  ...
  beforeFixedStep(dt: number, model: MatchModel, world?: World): void {
    ...
    if (this.world) {
      ...
      for (const rig of this.rigs.values()) for (let i = 0; i < rig.bodyEntries.length; i++) { const body = rig.bodyEntries[i].body; if (!body.isValid()) invalidRegisteredBodies += 1; }
  ...
  private advancePhysicalGrapple(world: World, model: MatchModel, dt: number): void {
    ...
    if (attacker.attackPhase === 'active') {
      ...
      for (let i = 0; i < defenderRig.bodyEntries.length; i++) {
        const body = defenderRig.bodyEntries[i].body;
        if (!body.isValid()) continue;
