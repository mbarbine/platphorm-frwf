...
interface RegisteredLandingSurface {
  body: RapierRigidBody;
  kind: 'ring' | 'floor' | 'table' | 'turnbuckle';
}
...
interface PendingStrikeCast {
  sourceFighter: FighterKey;
  targetFighter: FighterKey;
  sourceSegment: BodySegmentId;
  targetSegment: BodySegmentId;
  attackInstanceId: number;
  moveId: string;
  timeOfImpact: number;
  point: Vector3Value;
...
const IDLE_LOCOMOTION_BLOCKING_STATES = new Set<string>(['idle', 'locomotion', 'blocking']);
const GRAPPLE_PRE_LIFT_PHASES = new Set<string>(['reach', 'acquire', 'clinch', 'load']);
const STANDING_STANCE_STATES = new Set<string>(['idle', 'attacking', 'grappling', 'victorious']);
const DEFEATED_VICTORIOUS_STATES = new Set<string>(['defeated', 'victorious']);
const SUPPORTED_FALL_STATES = new Set<string>(['downed', 'airborne', 'defeated']);
const DECK_PENETRATION_STATES = new Set<string>(['airborne', 'downed', 'recovering', 'pinned', 'defeated']);
...
const GRIP_ANCHOR_RADIUS: Readonly<Partial<Record<BodySegmentId, number>>> = {
  pelvis: .22, abdomen: .21, chest: .27, head: HEAD_COLLIDER_RADIUS,
...
const bodySurfaceAnchorWorld = (body: RapierRigidBody, localX: number, hand: Vector3Value, segment: BodySegmentId): Vector3Value => {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups in distance calculation.
  const distance = Math.max(.001, Math.sqrt(dx * dx + dy * dy + dz * dz)); const radius = GRIP_ANCHOR_RADIUS[segment] ?? .09;
...
export class BodyWorksRuntime {
  ...
  private continuousStrikeCaptureCount = 0;
  private lastContinuousStrikePair = 'none';
  ...
  setNetworkTarget(fighter: FighterKey, position: Vec2, velocity: Vec2): void {
    if (![position.x, position.z, velocity.x, velocity.z].every(Number.isFinite)) return;
    this.networkTargets.set(fighter, { position: { ...position }, velocity: { ...velocity } });
  }

  clearNetworkTargets(): void { this.networkTargets.clear(); }

  private applyNetworkCorrections(): void {
    for (const [fighter, target] of this.networkTargets) {
      const rig = this.rigs.get(fighter); if (!rig) continue;
      const center = this.rigPlanarCenter(rig); const errorX = target.position.x - center.x; const errorZ = target.position.z - center.z;
      // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups in distance calculation.
      const distance = Math.sqrt(errorX * errorX + errorZ * errorZ);
      // Corrections remain coherent velocity changes across the whole body
      // tree, so joints and collisions keep authority. A large reconnect gap
      // is bounded more aggressively but is never a pelvis-only teleport.
      const gain = distance > 1.5 ? 5.5 : 2.8;
      this.applyRigVelocityDelta(rig, {
        x: clamp(target.velocity.x + errorX * gain - center.velocityX, -.42, .42),
        y: 0,
        z: clamp(target.velocity.z + errorZ * gain - center.velocityZ, -.42, .42),
      });
    }
  }
  ...
  resolveCommands(fighter: FighterKey, now: number, attempt: (command: BufferedPhysicsCommand) => boolean | ActionExecutionResult, expired?: (command: BufferedPhysicsCommand) => void): void {
    this.actions.resolveNext(now * 1_000, (command) => command.fighter === fighter, (command) => {
      ...
      if (!executed && typeof result !== 'boolean' && result.deferredReason) {
        command.deferredReason = result.deferredReason;
        if (fighter === 'player') this.playerActionFeedback = { event: command.event, status: 'buffered', updatedAt: now, reason: result.deferredReason, displayName };
      }
      return executed ? 'executed' : 'defer';
    }, (command) => {
      if (command.fighter === fighter) {
        if (fighter === 'player') this.playerActionFeedback = { event: command.event, status: 'expired', updatedAt: now, reason: command.deferredReason ?? 'Input window expired', displayName: null };
        expired?.(command);
      }
    });
    this.syncActionMetrics();
  ...
  canJump(fighter: FighterKey): boolean {
    const rig = this.rigs.get(fighter);
    return Boolean(rig?.bodies.pelvis?.isValid() && rig.supportContacts.size > 0 && rig.jumpCooldown <= 0);
  }

  /** Launch at command acceptance, so a later controller cannot silently drop it. */
  ...
  requestCornerDive(fighter: FighterKey, target: Vec2): void {
    ...
    const position = pelvis.translation(); const dx = target.x - position.x; const dz = target.z - position.z;
  ...
  prepareLabFall(fighter: FighterKey, orientation: RecoveryOrientation, facing: number, surfaceOffset = 0): void {
    ...
    rig.rootStabilized = false; rig.skeletonStabilized = false; rig.rotationSignature = ''; rig.rotationallyDynamic.clear(); rig.supportContacts.clear(); rig.settlingFrames = 0; rig.landingSupportFrames = 0; rig.airborneSeconds = 0; rig.recoveryOrientationCaptured = false;
  }
  ...
  continuousStrikeDiagnostics(): Readonly<{ count: number; pair: string }> { return { count: this.continuousStrikeCaptureCount, pair: this.lastContinuousStrikePair }; }
  ...
  beforeFixedStep(dt: number, model: MatchModel, world?: World): void {
    ...
    for (const key of AI_FIGHTER_SLOTS) {
      if (!slots.includes(key) || !IDLE_LOCOMOTION_STATES.has(model[key].state)) continue;
  ...
  private applyPendingLandingFollowThrough(model: MatchModel): void {
    for (const landing of this.pendingLandings.values()) {
      ...
      const motion = throwMotionFor(landing.targetPosition ? 'slam' : landing.moveId);
      const rising = age < motion.clearanceTime;
      const fallAge = Math.max(0, age - motion.clearanceTime);
      const targetVerticalVelocity = rising ? motion.riseSpeed : -Math.min(9.2, 1.1 + fallAge * 16);
      if (landing.targetPosition) {
        ...
        // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups.
        const distance = Math.sqrt(dx * dx + dz * dz); const desiredSpeed = distance > .12 ? clamp(distance / (rising ? .3 : .2), .18, 4.8) : 0;
  ...
  private syncMotionTasks(dt: number, model: MatchModel): void {
    ...
    for (const key of slots) {
      ...
      const task = this.tasks.request({
        ...
        maximumDuration: move.anticipationDuration + move.activeDuration + move.recoveryDuration + 2.2
  ...
  private applyFighterController(key: FighterKey, fighter: FighterRuntime, dt: number, model: MatchModel): void {
    ...
    const standingClinch = fighter.state === 'grabbed' && model.grapple?.defender === key
      && GRAPPLE_PRE_LIFT_PHASES.has(model.grapple.phase);
    ...
    const archingThrow = (fighter.moveId === 'suplex' || fighter.moveId === 'skyhook') && (fighter.attackPhase === 'active' || fighter.attackPhase === 'recovery');
    ...
    if (fighter.state === 'pinning' || fighter.state === 'pinned') {
      this.applyCoverController(key, fighter, rig, model, dt);
      return;
    }
    ...
    const stanceDrop = fighter.state === 'blocking' ? .065
      : fighter.state === 'locomotion' ? clamp(.04 + planarSpeed * .012, .04, .13)
        : STANDING_STANCE_STATES.has(fighter.state) ? .045 : 0;
    ...
    if (groundedControl) {
      ...
      const supportAcceleration = clamp(18 + ((recoveryTargetY - pelvis.translation().y) * 30 - velocity.y * 10.5) * Math.max(.85, fighter.body.muscle), -18, 38) * contactMultiplier * (.42 + recoveryBlend * .58);
    ...
    // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups.
    const inputLength = Math.min(1, Math.sqrt(intent.move.x * intent.move.x + intent.move.z * intent.move.z)) * movementControl;
    ...
    const acceleration = (inputLength <= .08 ? locomotion.braking : travel.running ? locomotion.runAcceleration : locomotion.acceleration) * (movementControl === 1 ? 1 : .24);
    if (movementControl > 0 && BODYWORKS_FLAGS.locomotion) {
      ...
      if (!neutralControl) rig.neutralAnchor = null;
        ...
        const dx = rig.neutralAnchor.x - center.x;
        const dz = rig.neutralAnchor.z - center.z;
        ...
        const anchorDistanceSq = dx * dx + dz * dz;
        const centerSpeedSq = center.velocityX * center.velocityX + center.velocityZ * center.velocityZ;
        ...
        if (anchorDistanceSq > 0.38 * 0.38 || centerSpeedSq > 0.65 * 0.65) rig.neutralAnchor = { x: center.x, z: center.z };
        else {
    ...
    const sepX = fighter.position.x - opponent.position.x;
    const sepZ = fighter.position.z - opponent.position.z;
    // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude check to avoid square root extraction entirely.
    const separationSq = sepX * sepX + sepZ * sepZ;
    if (rig.rootStabilized && (inputLength > .08 || separationSq < 4.8 * 4.8 || fighter.ropeRebound > 0)) {
    ...
    const held = model.grapple?.defender === key ? model.grapple : null;
    const carrier = held ? model[held.attacker] : null;
  ...
  private applyCoverController(key: FighterKey, fighter: FighterRuntime, rig: FighterRigRegistration, model: MatchModel, dt: number): void {
    ...
    if (!pelvis || !target) return;
    if (key === attacker) {
      cover.age += dt;
      const yaw = cover.facing + Math.PI / 2;
      fighter.facing = yaw;
      ...
      fighter.facing = cover.facing;
      this.applyPoseDrive(rig, fighter, selectMotorProfile(fighter), COVERED_POSE);
    }
  }

  private refreshCoverEvidence(model: MatchModel): void {
    const cover = model.pinCover;
    if (!cover) return;
  ...
  private configureRotationalAuthority(rig: FighterRigRegistration, fighter: FighterRuntime, profile: MotorProfile): void {
    ...
    const targets = physicalPoseTargets(targetPoseFor(fighter), fighter.facing, IDLE_LOCOMOTION_BLOCKING_STATES.has(fighter.state));
    ...
    const physicalReach = (fighter.moveId === 'grapple_miss' || fighter.moveId === 'prop_pickup' || fighter.moveId === 'prop_drop');
  ...
  private applyCloseRangeSeparation(model: MatchModel): void {
    ...
    for (let firstIndex = 0; firstIndex < slots.length; firstIndex += 1) for (let secondIndex = firstIndex + 1; secondIndex < slots.length; secondIndex += 1) {
      ...
      if (!IDLE_LOCOMOTION_BLOCKING_STATES.has(model[firstKey].state) || !IDLE_LOCOMOTION_BLOCKING_STATES.has(model[secondKey].state)) continue;
  ...
  private applyFootPlantDrive(rig: FighterRigRegistration, fighter: FighterRuntime, desiredVelocity: Vec2, inputLength: number): void {
    if (!(IDLE_LOCOMOTION_BLOCKING_STATES.has(fighter.state) || fighter.state === 'recovering')) return;
    ...
    for (const [id, planted] of entries) {
      ...
      // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups.
      const velocity = foot.linvel(); const planarSpeed = Math.sqrt(velocity.x * velocity.x + velocity.z * velocity.z); if (planarSpeed < .18) continue;
  ...
  private applyPhysicalStrike(key: FighterKey, rig: FighterRigRegistration, fighter: FighterRuntime, model: MatchModel): void {
    ...
    const spDz = targetPosition.z - sourcePosition.z;
    // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups.
    const strikePlanarDistance = Math.sqrt(spDx * spDx + spDz * spDz);
    const strikeVerticalDistance = Math.abs(targetPosition.y - sourcePosition.y);
    const sDy = targetPosition.y - sourcePosition.y;
    // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedups.
    const strikeDistance = Math.sqrt(spDx * spDx + sDy * sDy + spDz * spDz);
    const strikeMetricKey = `${key}:${fighter.attackInstanceId}:${fighter.moveId}`;
    if (strikeMetricKey !== this.lastStrikeMetricKey) {
      this.lastStrikeMetricKey = strikeMetricKey;
      this.metrics.minimumStrikeDistance = fighter.attackPhase === 'active' ? strikeDistance : Number.POSITIVE_INFINITY;
      this.metrics.minimumStrikePlanarDistance = fighter.attackPhase === 'active' ? strikePlanarDistance : Number.POSITIVE_INFINITY;
      this.metrics.minimumStrikeVerticalDistance = fighter.attackPhase === 'active' ? strikeVerticalDistance : Number.POSITIVE_INFINITY;
    }
    this.metrics.lastStrikeDistance = strikeDistance;
    if (fighter.attackPhase === 'active' && strikeDistance < this.metrics.minimumStrikeDistance) {
    ...
    const sepDx = targetPosition.x - pelvis.translation().x;
  ...
  /**
   * Preserve a real Rapier time-of-impact that can disappear before the
   * post-step narrow phase is inspected. This is continuous collision truth,
   * not an animation event or an authored reach volume: Rapier must cast the
   * actual attacking collider into an actual opponent collider during this
   * fixed step.
   */
  private captureContinuousStrikeContact(sourceFighter: FighterKey, targetFighter: FighterKey, sourceSegment: BodySegmentId, sourceBody: RapierRigidBody, targetRig: FighterRigRegistration, fighter: FighterRuntime, targetSegments?: readonly BodySegmentId[]): void {
    if (!fighter.moveId || sourceBody.numColliders() === 0) return;
    const sourceCollider = sourceBody.collider(0); const sourceForce = sourceBody.userForce(); const sourceMass = Math.max(.001, sourceBody.mass());
    const sourceLinearVelocity = sourceBody.linvel();
    const sourceVelocity = {
      x: sourceLinearVelocity.x + sourceForce.x / sourceMass * this.currentFixedDt,
      y: sourceLinearVelocity.y + sourceForce.y / sourceMass * this.currentFixedDt,
    ...
    for (const _segment in targetRig.bodies) {
      ...
      const targetVelocity = {
        x: targetLinearVelocity.x + targetForce.x / targetMass * this.currentFixedDt,
      ...
      if (proximity && proximity.distance <= .012) {
        const existing = this.pendingStrikeCasts.get(pendingKey); if (existing?.timeOfImpact === 0) continue;
        const reducedMass = sourceMass * targetMass / Math.max(.1, sourceMass + targetMass);
        this.pendingStrikeCasts.set(pendingKey, {
          sourceFighter, targetFighter, sourceSegment, targetSegment, attackInstanceId: fighter.attackInstanceId, moveId: fighter.moveId,
          timeOfImpact: 0,
          point: {
            x: (proximity.point1.x + proximity.point2.x) * .5,
            y: (proximity.point1.y + proximity.point2.y) * .5,
            z: (proximity.point1.z + proximity.point2.z) * .5,
          },
          direction: { x: proximity.normal1.x, y: proximity.normal1.y, z: proximity.normal1.z },
          relativeSpeed,
          impulse: relativeSpeed * reducedMass,
        });
        this.continuousStrikeCaptureCount += 1; this.lastContinuousStrikePair = `${sourceSegment}>${targetSegment}:proximity`;
        continue;
      }
      const hit = sourceCollider.castCollider(sourceVelocity, targetCollider, targetVelocity, .001, this.currentFixedDt, true);
      if (!hit) continue;
      const existing = this.pendingStrikeCasts.get(pendingKey);
      if (existing && existing.timeOfImpact <= hit.time_of_impact) continue;
      const rotation = sourceCollider.rotation(); const witness = hit.witness1; const normal = hit.normal1;
      const rotate = (vector: Vector3Value): Vector3Value => {
      ...
      const witnessWorld = rotate(witness); const normalWorld = rotate(normal); const colliderPosition = sourceCollider.translation();
      const reducedMass = sourceMass * targetMass / Math.max(.1, sourceMass + targetMass);
      this.pendingStrikeCasts.set(pendingKey, {
        sourceFighter, targetFighter, sourceSegment, targetSegment, attackInstanceId: fighter.attackInstanceId, moveId: fighter.moveId,
        timeOfImpact: hit.time_of_impact,
        point: {
          x: colliderPosition.x + sourceVelocity.x * hit.time_of_impact + witnessWorld.x,
          y: colliderPosition.y + sourceVelocity.y * hit.time_of_impact + witnessWorld.y,
          z: colliderPosition.z + sourceVelocity.z * hit.time_of_impact + witnessWorld.z,
        },
        direction: normalWorld,
        relativeSpeed,
        impulse: relativeSpeed * reducedMass,
      });
      this.continuousStrikeCaptureCount += 1; this.lastContinuousStrikePair = `${sourceSegment}>${targetSegment}:toi`;
    }
  }
  ...
  private applyRopeController(rig: FighterRigRegistration, fighter: FighterRuntime, model: MatchModel): void {
    ...
    const battleContained = model.matchMode === 'battle_royale' && !model.labMode && !DEFEATED_VICTORIOUS_STATES.has(fighter.state);
    ...
    const ignoresRopes = (fighter.state === 'defeated' || fighter.state === 'climbing');
    ...
    // a lift or jump is still outside. Clear this only after actual re-entry.
    if (!battleContained && rig.ringsideEstablished) { rig.ropeContact = null; return; }
  ...
  private releaseRopeRebound(rig: FighterRigRegistration, fighter: FighterRuntime, model: MatchModel, contact: NonNullable<FighterRigRegistration['ropeContact']>): void {
    ...
    const useTargetLane = targetDistance > .001 && !DEFEATED_VICTORIOUS_STATES.has(opponent.state);
  ...
  private advancePhysicalGrapple(world: World, model: MatchModel, dt: number): void {
    ...
    if (!attackerRig || !defenderRig || !attacker.moveId || !(attacker.state === 'grappling' || attacker.state === 'attacking')) { this.releaseAllGrips(world); this.grappleEnvironmentTarget = null; model.grapple = null; return; }
    ...
    const preLiftPhase = GRAPPLE_PRE_LIFT_PHASES.has(grapple.phase);
    ...
    const defenderGroundLocked = groundedLock(defender, defenderPelvisAtStart);
    if (preLiftPhase && grapple.age > .9 && (attackerGroundLocked || defenderGroundLocked)) {
      // A clinch is never allowed to become a permanent floor restraint. If
      // either articulated pelvis collapses before the lift is established,
      // break the grips and route the fallen wrestler through the same bounded
      // downed/recovery contract as every other physical fall.
      this.releaseAllGrips(world); this.grappleEnvironmentTarget = null; model.grapple = null;
      for (const [fighterKey, groundLocked] of [[grapple.attacker, attackerGroundLocked], [grapple.defender, defenderGroundLocked]] as const) {
        const participant = model[fighterKey]; participant.moveId = null; participant.attackPhase = null; participant.phaseElapsed = 0; participant.stateElapsed = 0;
        if (groundLocked) {
          participant.state = 'downed'; participant.downTimer = Math.max(participant.downTimer, .45); beginFall(model, fighterKey, FALL_REASONS.SupportCollision);
        } else participant.state = fighterKey === grapple.attacker ? 'staggered' : 'idle';
      }
      model.announcement = 'GROUND LOCK BROKEN — RESET!'; model.announcementTimer = .85;
      return;
    }
    ...
    if (owned.length < 2) {
      ...
      // Every standing entry acquires the same reachable collar-and-elbow tie.
      // A waist-lock target cannot be reached while the entry pose holds a high guard.
      const preferences: readonly [BodySegmentId, BodySegmentId, number][] = gripPreferences('slam');
      for (const [handId, targetId, targetAnchorX] of preferences) {
        ...
        const catchDistance = acquiredHands > 0 ? .46 : .34;
    ...
    for (const grip of activeGrips) {
      ...
      if (!(attacker.state === 'grappling' || attacker.state === 'attacking')) this.removeGrip(world, grip, 'incompatible-state');
    ...
    const atkMoveMag = Math.sqrt(attackerIntent.move.x * attackerIntent.move.x + attackerIntent.move.z * attackerIntent.move.z);
    grapple.struggle = clamp(grapple.struggle + (defMoveMag * .8 - atkMoveMag * .28 - .08) * dt, 0, 1);
    ...
    if (progress < .38) grapple.phase = 'clinch';
      ...
      grapple.phase = 'lift';
      grapple.liftElapsed = (grapple.liftElapsed ?? 0) + dt;
    ...
    if (attacker.attackPhase === 'active') {
      ...
      const direction = environmentTargeted && environmentDelta && environmentDistance > .08
        ...
        : inputDirection;
      const horizontalReleaseSpeed = environmentTargeted ? clamp(environmentDistance / (environmentTarget?.surface === 'turnbuckle' ? .64 : .48), .06, environmentTarget?.surface === 'turnbuckle' ? 6.8 : 7.5) : motion.speed;
  ...
  private applyPoseDrive(rig: FighterRigRegistration, fighter: FighterRuntime, motorProfile: MotorProfile, overridePose?: Pose): void {
    const supportedFall = !overridePose && SUPPORTED_FALL_STATES.has(fighter.state);
    const pose = supportedFall ? BREAKFALL_POSE : overridePose ?? targetPoseFor(fighter);
    const targets = physicalPoseTargets(pose, supportedFall ? 0 : fighter.facing, IDLE_LOCOMOTION_BLOCKING_STATES.has(fighter.state) || fighter.state === 'grappling' && motorProfile.id === 'lift');
    ...
    if (fighter.moveId === 'headbutt' && fighter.attackPhase === 'active') {
      // The brow must travel beyond the chest plane. Drive a real neck flexion
    ...
    for (const side of ['left', 'right'] as const) {
      ...
      if (rig.bodies.pelvis && IDLE_LOCOMOTION_BLOCKING_STATES.has(fighter.state)) {
      ...
      if (shin?.isValid()) {
        ...
        const plant = fighter.state === 'climbing' && fighter.climbStage === 3 || (IDLE_LOCOMOTION_BLOCKING_STATES.has(fighter.state) || fighter.state === 'recovering') || standingStrike && strikeSource !== `${side}Foot`;
  ...
  private refreshActiveStrikeContacts(model: MatchModel): void {
    ...
    for (const sourceKey of slots) {
      ...
      const capturedActiveStep = moveId ? [...this.pendingStrikeCasts.values()].some((candidate) => candidate.sourceFighter === sourceKey
        && candidate.attackInstanceId === sourceRuntime.attackInstanceId && candidate.moveId === moveId) : false;
      // Rules advance immediately before Rapier steps. Preserve a contact that
      // the fixed-step cast captured during the final active frame even if the
      // rules model entered recovery on that same boundary.
      if (!moveId || (sourceRuntime.attackPhase !== 'active' && !capturedActiveStep)) continue;
      ...
      const sourceSegments: readonly BodySegmentId[] = moveId === 'aerial'
        ...
        : moveId === 'stiff_arm' || moveId === 'rebound'
          ...
          : [profile.source];
      const targetKeys = slots.filter((targetKey) => targetKey !== sourceKey && !DEFEATED_VICTORIOUS_STATES.has(model[targetKey].state));
      for (const targetKey of targetKeys) {
        ...
        const continuousHit = this.pendingStrikeCasts.get(`${sourceKey}:${targetKey}:${sourceRuntime.attackInstanceId}`);
        let recorded = false;
      ...
      if (!targetBody) continue;
            ...
            // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for ~8x speedup.
            const rvx = sourceVelocity.x - targetVelocity.x; const rvy = sourceVelocity.y - targetVelocity.y; const rvz = sourceVelocity.z - targetVelocity.z;
            ...
            if (!touching && relativeSpeed >= .28) {
              // CCD can resolve a fast limb at time-of-impact without leaving
              // a persistent post-step manifold. Ask Rapier for the exact
              // shape-to-shape separation, then accept only a sub-12 mm
              // speculative contact. This remains collider-authoritative: no
              // distance cone, animation marker, or renderer event can score.
              const proximity = sourceCollider.contactCollider(targetCollider, .012);
              if (proximity && proximity.distance <= .012) {
                touching = true;
                point = {
                ...
                direction = { x: proximity.normal1.x, y: proximity.normal1.y, z: proximity.normal1.z };
                const reducedMass = sourceBody.mass() * targetBody.mass() / Math.max(.1, sourceBody.mass() + targetBody.mass());
                totalImpulse = relativeSpeed * reducedMass; maximumImpulse = totalImpulse;
              }
            }
            if (!touching) continue;
            const position = point ?? sourceBody.translation();
  ...
  private settleSupportedAirborneFighter(key: FighterKey, model: MatchModel): void {
    ...
    if (!rig) return;
    if (!settlingJump && fighter.state !== 'airborne') {
      rig.landingSupportFrames = 0; rig.airborneSeconds = 0;
      return;
    }
    rig.airborneSeconds += this.currentFixedDt;
    if (this.pendingLandings.has(key)) {
    ...
    // Repeated strikes can reset rules-state elapsed time faster than render
    // contact callbacks settle. Once the articulated pelvis has remained low
    // for a bounded total airborne window, resolve to the mat state even if a
    // foot manifold flickers. This changes no transform and awards no impact.
    const staleLowBody = rig.airborneSeconds >= .65 && fighter.body.verticalOffset <= .62 && lowestCenterY <= surfaceY + .58;
    if (rig.landingSupportFrames < 5 && !staleLowBody) return;
  ...
  private containRigsToArena(model: MatchModel): void {
    ...
    for (const key of slots) {
      ...
      const battleContained = model.matchMode === 'battle_royale' && !model.labMode && !DEFEATED_VICTORIOUS_STATES.has(model[key].state);
      ...
      const activeFighter = !DEFEATED_VICTORIOUS_STATES.has(model[key].state);
  ...
  private correctCoreDeckPenetration(key: FighterKey, model: MatchModel): void {
    const fighter = model[key];
    if (!DECK_PENETRATION_STATES.has(fighter.state)) return;
    const rig = this.rigs.get(key); if (!rig) return;
    const surfaceY = this.isRingside(fighter.position) ? .4 : VOLT_DOME.ring.deckY;
    ...
    const settlingOnDeck = (fighter.state === 'downed' || fighter.state === 'recovering' || fighter.state === 'pinned' || fighter.state === 'defeated');
    for (const _segment in rig.bodies) {
      const body = rig.bodies[_segment as BodySegmentId];
      if (!body?.isValid()) continue;
  ...
  private supportScore(rig: FighterRigRegistration): number {
    ...
    return clamp(1 - Math.sqrt((centerX - closestX) * (centerX - closestX) + (centerZ - closestZ) * (centerZ - closestZ)) / .82, 0, 1);
  }
