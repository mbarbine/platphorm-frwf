...
import { ALL_BODY_SEGMENT_IDS, CORE_SEGMENTS, HEAD_COLLIDER_RADIUS, buildBodySchema, torsoColliderArgs } from './bodySchema';
...
export class BodyWorksRuntime {
  ...
  registerFighter(fighter: FighterKey, bodies: Partial<Record<BodySegmentId, RapierRigidBody>>, jointCount: number): () => void {
    ...
    for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) {
      const segment = ALL_BODY_SEGMENT_IDS[i];
  ...
  private rigPlanarCenter(rig: FighterRigRegistration): { x: number; z: number; velocityX: number; velocityZ: number; mass: number } {
    ...
    for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) {
      const body = rig.bodies[ALL_BODY_SEGMENT_IDS[i]];
  ...
  beforeFixedStep(dt: number, model: MatchModel, world?: World): void {
    ...
    if (this.world) {
      ...
      for (const rig of this.rigs.values()) for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) { const body = rig.bodies[ALL_BODY_SEGMENT_IDS[i]]; if (body && !body.isValid()) invalidRegisteredBodies += 1; }
  ...
  private applyFighterController(key: FighterKey, fighter: FighterRuntime, dt: number, model: MatchModel): void {
    ...
    if (movementControl > 0 && BODYWORKS_FLAGS.locomotion) {
      ...
      if (!settled) for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) {
        const body = rig.bodies[ALL_BODY_SEGMENT_IDS[i]];
        if (!body?.isValid()) continue;
  ...
  private captureContinuousStrikeContact(sourceFighter: FighterKey, targetFighter: FighterKey, sourceSegment: BodySegmentId, sourceBody: RapierRigidBody, targetRig: FighterRigRegistration, fighter: FighterRuntime, targetSegments?: readonly BodySegmentId[]): void {
    ...
    for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) {
      const targetSegment = ALL_BODY_SEGMENT_IDS[i];
  ...
  private advancePhysicalGrapple(world: World, model: MatchModel, dt: number): void {
    ...
    if (attacker.attackPhase === 'active') {
      ...
      for (let i = 0; i < ALL_BODY_SEGMENT_IDS.length; i++) {
        const body = defenderRig.bodies[ALL_BODY_SEGMENT_IDS[i]];
        if (!body?.isValid()) continue;
