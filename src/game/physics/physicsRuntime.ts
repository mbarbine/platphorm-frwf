...
import { BODY_SEGMENT_IDS, CORE_SEGMENTS, HEAD_COLLIDER_RADIUS, buildBodySchema, torsoColliderArgs } from './bodySchema';
...
interface FighterRigRegistration {
  ...
  bodyList: readonly RapierRigidBody[];
  bodyEntries: readonly (readonly [BodySegmentId, RapierRigidBody])[];
...
export class BodyWorksRuntime {
  ...
  registerFighter(fighter: FighterKey, bodies: Partial<Record<BodySegmentId, RapierRigidBody>>, jointCount: number): () => void {
    ...
    const bodyList: RapierRigidBody[] = [];
    const bodyEntries: [BodySegmentId, RapierRigidBody][] = [];
  ...
  private recount(fallbackJoints: number): void {
    let bodies = 0;
    for (const rig of this.rigs.values()) bodies += rig.bodyList?.length ?? Object.keys(rig.bodies).length;
  ...
  private rigPlanarCenter(rig: FighterRigRegistration): { x: number; z: number; velocityX: number; velocityZ: number; mass: number } {
    ...
    // OPTIMIZATION: Zero-allocation indexed loop over pre-cached bodyList for hot-path physics calculation
  ...
  prepareLabFall(fighter: FighterKey, orientation: RecoveryOrientation, facing: number, surfaceOffset = 0): void {
    ...
    for (let i = 0; i < rig.bodyEntries.length; i++) {
      const [segment, body] = rig.bodyEntries[i] as readonly [BodySegmentId, RapierRigidBody];
  ...
  beforeFixedStep(dt: number, model: MatchModel, world?: World): void {
    ...
    if (this.world) {
      ...
      for (const rig of this.rigs.values()) for (let i = 0; i < rig.bodyList.length; i++) { const body = rig.bodyList[i]; if (body && !body.isValid()) invalidRegisteredBodies += 1; }
  ...
  private applyFighterController(key: FighterKey, fighter: FighterRuntime, dt: number, model: MatchModel): void {
    ...
    if (movementControl > 0 && BODYWORKS_FLAGS.locomotion) {
      ...
      if (!settled) for (let i = 0; i < rig.bodyList.length; i++) {
        const body = rig.bodyList[i];
        if (!body?.isValid()) continue;
  ...
  private configureRotationalAuthority(rig: FighterRigRegistration, fighter: FighterRuntime, profile: MotorProfile): void {
    ...
    if (profile.rootMode === 'physical') for (let i = 0; i < BODY_SEGMENT_IDS.length; i++) { const seg = BODY_SEGMENT_IDS[i]; if (seg) dynamic.add(seg); }
