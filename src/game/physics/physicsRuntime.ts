...
export class BodyWorksRuntime {
  ...
  private recount(fallbackJoints: number): void {
    let bodies = 0;
    for (const rig of this.rigs.values()) for (const segment of ALL_BODY_SEGMENTS) { if (rig.bodies[segment]) bodies++; }
  ...
  beforeFixedStep(dt: number, model: MatchModel, world?: World): void {
    ...
    if (this.world) {
      ...
      for (const rig of this.rigs.values()) for (const segment of ALL_BODY_SEGMENTS) { const body = rig.bodies[segment]; if (body && !body.isValid()) invalidRegisteredBodies += 1; }
