...
const clampMagnitude = (vector: PhysicsVector3, maximum: number): PhysicsVector3 => {
  const sq = vector.x * vector.x + vector.y * vector.y + vector.z * vector.z;
  // OPTIMIZATION: Replacing slow Math.hypot with pre-squared magnitude checks.
  if (sq <= maximum * maximum || sq < 1e-14) return vector;
...
export const sweptPlanarPathHitsTarget = (start: PhysicsVector2, end: PhysicsVector2, target: PhysicsVector2, width: number): boolean => {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude check to avoid Math.sqrt entirely.
