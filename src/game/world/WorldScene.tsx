...
export function WorldScene({ onEncounter, onExit }: { onEncounter: (encounter: WorldEncounter) => void; onExit: () => void }) {
  ...
  const [paused, setPaused] = useState(false); const [offer, setOffer] = useState<WorldEncounter | null>(null); const [lost, setLost] = useState(false); const [map, setMap] = useState(false);
  ...
  const disabled = paused || !!offer || lost;
  return <section className="world-screen" data-testid="showground" data-region={region} data-world-x={save.position.x.toFixed(2)} data-world-z={save.position.z.toFixed(2)}>
    ...
    {offer && <div className="world-modal" role="dialog" aria-modal="true" aria-label={offer.title}><article><span>MEET {fighterById(offer.host).name}</span><h2>{offer.title}</h2><p>{offer.description}</p><p className="world-offer-detail">This bout takes place in the Volt Dome. You’ll return here afterward.</p>{save.results[offer.id] && <p>Your record: {save.results[offer.id]?.wins} wins / {save.results[offer.id]?.bouts} completed bouts</p>}<button className="button button--hero" onClick={() => { if (useWorldSession.getState().begin(offer.id)) onEncounter(offer); }}>START BOUT</button><button className="button button--quiet" onClick={() => setOffer(null)}>KEEP EXPLORING</button></article></div>}
