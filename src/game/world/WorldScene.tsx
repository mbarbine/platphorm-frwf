import { PushMower, LawnmowerGame, nearLawnmower } from './LawnmowerActivity';
...
export function WorldScene({ onEncounter, onExit }: { onEncounter: (encounter: WorldEncounter) => void; onExit: () => void }) {
  ...
  const [mower, setMower] = useState(false);
  ...
  const interact = useCallback(() => { if (nearLawnmower(useWorldSession.getState().save.position)) { useWorldSession.getState().checkpoint(); setMower(true); return; } const encounter = nearbyEncounter(useWorldSession.getState().save.position); if (encounter) { setOffer(encounterForFighter(encounter, useWorldSession.getState().save.fighter)); useWorldSession.getState().checkpoint(); } }, []);
  useEffect(() => {
  ...
  const disabled = mower || archive || paused || !!offer || lost;
  return <section className="world-screen" data-testid="showground" data-region={region} data-world-x={save.position.x.toFixed(2)} data-world-z={save.position.z.toFixed(2)}>
    <WorldBoundary onExit={onExit}><Canvas shadows={quality !== 'performance'} dpr={[1, quality === 'performance' ? 1 : 1.5]} camera={{ position: [0, 11, 33], fov: 48 }}>
      ...
      <ShowgroundEnvironment /><PushMower />
    ...
    {!disabled && <div className="world-prompt">{nearLawnmower(save.position) ? <button onClick={interact}><span>F / A · MOWER</span><b>PLAY LAWNMOWER</b></button> : nearby ? <button onClick={interact}><span>F / A · TALK & WRESTLE</span><b>{nearby.title}</b></button> : <p>WASD / left stick to explore · Shift to run<br /><span>Walk up to a wrestler to start a bout.</span></p>}</div>}
    <WorldTouch paused={disabled} />
    {mower && <LawnmowerGame onClose={() => setMower(false)} />}
