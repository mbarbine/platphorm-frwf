import { CIRCUIT_OBJECTIVES, circuitProgress, earnedMedals, type CircuitMedal } from './circuit';
import { create } from 'zustand';
import { FIGHTERS } from '../data/fighters';
import type { FighterId, Vec2, MatchResult } from '../types/game';
...
export interface WorldSave { version: 1; fighter: FighterId; position: Vec2; facing: number; visited: RegionId[]; results: Record<string, { bouts: number; wins: number }>; medals: Record<string, CircuitMedal[]> }
export const freshWorld = (fighter: FighterId = 'atlas'): WorldSave => ({ version: 1, fighter, position: { ...WORLD_START }, facing: Math.PI, visited: ['showground'], results: {}, medals: {} });
export function parseWorldSave(raw: string | null): WorldSave {
  try {
    ...
    for (const encounter of WORLD_ENCOUNTERS) {
      ...
      const medals = s.medals?.[encounter.id];
      ...
      if (result.results[encounter.id]?.wins) result.medals[encounter.id] = CIRCUIT_OBJECTIVES.map(o => o.id).filter(id => id === 'victory' || (Array.isArray(medals) && medals.includes(id)));
    }
    return result;
...
interface WorldState {
  ...
  finish: (won: boolean, result?: MatchResult) => void;
  abandon: () => void;
}
export const useWorldSession = create<WorldState>((set, get) => ({
  ...
  finish: (won, result) => {
    ...
    set(state => { const before = state.save.results[id] ?? { bouts: 0, wins: 0 }; return { activeEncounter: null, save: { ...state.save, medals: { ...state.save.medals, [id]: [...new Set([...(state.save.medals[id] ?? []), ...(result ? earnedMedals(result) : [])])] }, results: { ...state.save.results, [id]: { bouts: Math.min(100000, before.bouts + 1), wins: Math.min(100000, before.wins + (won ? 1 : 0)) } } } }; });
    get().checkpoint();
  },
