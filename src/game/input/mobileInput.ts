...
export const mobileInput = {
  ...
  read(): FrameInput & { active: boolean } {
    ...
    // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude check (> 0.0064 equivalent to > 0.08) to eliminate square root computation on touch polling.
    const moveEvent = state.held.update('move', (state.move.x * state.move.x + state.move.z * state.move.z) > 0.0064, 'touch', state.move, timestamp);
