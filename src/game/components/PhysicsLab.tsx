...
export function PhysicsLab() {
  ...
  const run = (scenario: LabScenario): void => {
    ...
    const closeRange = ['jab', 'hook', 'guard', 'kick', 'lock', 'slam', 'failedLift', 'gripBreak', 'suplex', 'powerbomb', 'clothesline', 'spear', 'soakRound'].includes(scenario.id);
  ...
  const applyPair = (): void => { clearTimers(); setActive(null); useMatchStore.getState().configureLab(playerId, opponentId, seed, playerStamina, opponentStamina); };
  const stepOnce = (): void => {
  ...
  const diagnostics = useMemo(() => [
    ...
    ['CONTACTS', metrics.contactCount], ['FORCE / LOAD', `${model.lastImpact?.force?.toFixed(1) ?? '0'} / ${metrics.maximumGripLoad.toFixed(1)}`], ['RESETS / BOUNDS', `${metrics.emergencyResetCount} / ${metrics.containmentCount}`], ['REPLAY', `${bodyWorksRuntime.replay.size} · ${(metrics.replayEstimatedBytes / 1024).toFixed(0)} KB`],
  ] as const, [fps, metrics.averageStepMs, metrics.bodyCount, metrics.contactCount, metrics.containmentCount, metrics.emergencyResetCount, metrics.gripCount, metrics.gripCreateCount, metrics.jointCount, metrics.maximumGripLoad, metrics.maximumStepMs, metrics.p95StepMs, metrics.replayEstimatedBytes, metrics.worldBodyCount, metrics.worldJointCount, model, opponent.pelvisY, player.pelvisY, player.speed, player.supportFeet, player.upright, revision]);
  return <aside className="physics-lab" data-testid="physics-lab" data-lab-fps={fps} data-lab-step-ms={metrics.lastStepMs.toFixed(3)} data-lab-avg-step-ms={metrics.averageStepMs.toFixed(3)} data-lab-p95-step-ms={metrics.p95StepMs.toFixed(3)} data-lab-max-step-ms={metrics.maximumStepMs.toFixed(3)} data-lab-rate={rate} data-lab-debug={debug ? 'true' : 'false'}>
    <header><span>RINGFALL BODYWORKS</span><b>PHYSICS LAB</b><small>REAL INPUT · REAL RAPIER · FIXED 60 HZ AUTHORITY</small></header>
    ...
    <details className="physics-lab__setup"><summary>PAIR / SEED / STAMINA</summary><div><label>PLAYER<select value={playerId} onChange={(event) => setPlayerId(event.target.value as FighterId)}>{FIGHTERS.map((fighter) => <option key={fighter.id} value={fighter.id}>{fighter.name}</option>)}</select></label><label>OPPONENT<select value={opponentId} onChange={(event) => setOpponentId(event.target.value as FighterId)}>{FIGHTERS.map((fighter) => <option key={fighter.id} value={fighter.id}>{fighter.name}</option>)}</select></label><label>SEED<input type="number" value={seed} onChange={(event) => setSeed(Number(event.target.value) || 1)} /></label><label>P1 GAS {playerStamina}%<input type="range" min="10" max="100" step="5" value={playerStamina} onChange={(event) => setPlayerStamina(Number(event.target.value))} /></label><label>CPU GAS {opponentStamina}%<input type="range" min="10" max="100" step="5" value={opponentStamina} onChange={(event) => setOpponentStamina(Number(event.target.value))} /></label><button onClick={applyPair}>LOAD PAIR</button></div></details>
