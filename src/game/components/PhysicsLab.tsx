...
export function PhysicsLab() {
  ...
  return <aside className="physics-lab" data-testid="physics-lab" data-lab-scenario={active ?? 'idle'} data-lab-fps={fps} data-lab-step-ms={metrics.lastStepMs.toFixed(3)} data-lab-avg-step-ms={metrics.averageStepMs.toFixed(3)} data-lab-p95-step-ms={metrics.p95StepMs.toFixed(3)} data-lab-max-step-ms={metrics.maximumStepMs.toFixed(3)} data-lab-replay-kb={(metrics.replayEstimatedBytes / 1024).toFixed(1)} data-lab-joint-separation={metrics.maximumJointSeparation.toFixed(3)} data-lab-numerical-faults={metrics.numericalFaultCount} data-lab-support-score={metrics.supportScore.toFixed(3)} data-lab-rate={rate} data-lab-debug={debug ? 'true' : 'false'}>
