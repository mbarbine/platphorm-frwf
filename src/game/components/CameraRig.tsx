...
export function CameraRig() {
  ...
  useFrame((_, dt) => {
    ...
    // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt to avoid dynamic scaling checks on frame path.
    const framingDx = maximumX - minimumX;
    const framingDz = maximumZ - minimumZ;
    const separation = Math.sqrt(framingDx * framingDx + framingDz * framingDz);
    ...
    if (impact && impact.id !== impactId.current) {
      ...
      // OPTIMIZATION: Zero-allocation squared-magnitude comparison (< 5.76 equivalent to < 2.4) to avoid Math.sqrt extraction.
