...
export const resolveContextAction = (model: MatchModel, actorKey: FighterSlot, direction: Vec2 = { x: 0, z: 0 }): ResolvedContextAction => {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude comparison to avoid slow square-root extraction.
  if (actor.state === 'grappling' && actor.attackPhase === 'anticipation' && model.grapple?.attacker === actorKey && (dx * dx + dz * dz) <= 9.9225) {
  ...
  if (target.state === 'downed' && separation <= 1.7) {
    // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude comparison.
