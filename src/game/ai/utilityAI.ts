...
export const isActionLegal = (model: MatchModel, command: GameCommand, actorKey: FighterSlot): boolean => {
  ...
  if (actor.state === 'grappling' && actor.attackPhase === 'anticipation' && command === 'context') {
    ...
    // OPTIMIZATION: Replacing slow Math.hypot with a zero-allocation squared-magnitude check (3.15^2 = 9.9225).
...
export const chooseAiDecision = (model: MatchModel, definition: FighterDefinition, actorKey: FighterSlot = 'opponent'): AiDecision => {
  ...
  // OPTIMIZATION: Replacing allocation-heavy .filter().sort() with single-pass O(N) loop and zero-allocation squared comparisons.
  let propTarget;
  if (model.ruleset === 'chaos' && !actor.heldPropId) {
    ...
    for (let i = 0; i < model.props.length; i++) {
      ...
      if (!prop.broken && !prop.heldBy && prop.kind !== 'table') {
        const dx = prop.position.x - actor.position.x;
        const dz = prop.position.z - actor.position.z;
  ...
  const pursuesProp = Boolean(propTarget && (actorKey === 'opponent' && model.elapsed > .75
