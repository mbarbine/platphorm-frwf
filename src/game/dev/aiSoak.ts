...
const toward = (from: Vec2, to: Vec2): Vec2 => {
  const x = to.x - from.x; const z = to.z - from.z;
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for AI soak direction calculation.
  const magnitude = Math.max(.001, Math.sqrt(x * x + z * z));
...
const playerBotInput = (model: MatchModel, step: number): FrameInput => {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for separation calculation.
  const dxSep = target.position.x - actor.position.x;
  const dzSep = target.position.z - actor.position.z;
  const separation = Math.sqrt(dxSep * dxSep + dzSep * dzSep);
