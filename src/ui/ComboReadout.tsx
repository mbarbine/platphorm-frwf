...
export function ComboReadout({ actor, punch, kick }: { actor: FighterRuntime; punch: string; kick: string }) {
  ...
  return <div className="hit-combo" role="status" aria-live="polite" aria-label={actor.comboStep >= 1 ? `Combo: ${actor.comboStep} ${comboLabel}` : undefined} data-testid="hit-combo" data-hits={actor.comboStep} data-chain={actor.comboInputs.join(',')} data-combo-name={actor.comboName ?? ''} hidden={actor.comboStep < 1}>
