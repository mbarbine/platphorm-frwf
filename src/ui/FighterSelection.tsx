import provisionalModels from '../../tools/characters/provisional-models.json';
...
export function FighterSelection({ selected, onSelect, onBack, onConfirm }: Props) {
  ...
  return <section className="selection-arena" style={{ '--fighter-accent': fighter.palette.primary } as CSSProperties} aria-label="Choose your wrestler">
    ...
    <div className="selection-stage">
      ...
      <div className="selection-model"><span className="selection-model-label">{selected in provisionalModels ? 'PROVISIONAL MODEL · LIKENESS IN PROGRESS' : 'IN THE RING'}</span><Suspense fallback={<div className="preview-loading">ASSEMBLING FIGHTER…</div>}><FighterPreview fighterId={selected} /></Suspense></div>
