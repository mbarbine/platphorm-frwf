...
export function MobileControls({ onPause, paused }: MobileControlsProps) {
  ...
  // OPTIMIZATION: Zero-allocation squared-distance check avoids expensive Math.hypot / Math.sqrt calls on render frame
  const targetDx = player.position.x - opponent.position.x;
  const targetDz = player.position.z - opponent.position.z;
  const inGrappleRange = (targetDx * targetDx + targetDz * targetDz) <= (GRAPPLE_ACQUISITION_RANGE * GRAPPLE_ACQUISITION_RANGE);
  ...
  const grappleLabel = player.state === 'climbing' || player.state === 'downed' || player.state === 'pinned' ? 'NO LOCK'
    : grappleMove ? getMove(grappleMove).displayName.toUpperCase()
      : inGrappleRange ? 'VOLTAGE SLAM' : 'COLLAR REACH (MISS)';
  ...
  const moveStick = (clientX: number, clientY: number): void => {
    ...
    // OPTIMIZATION: Zero-allocation squared check avoids Math.hypot, using standard Math.sqrt only when magnitude exceeds 1
