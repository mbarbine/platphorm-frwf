...
export function buildControlLabels(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, direction: Vec2 = { x: 0, z: 0 }, running = false): Readonly<Record<ControlId, string>> {
  ...
  // OPTIMIZATION: Replace slow Math.hypot with standard Math.sqrt and zero-allocation squared comparison.
  const dxClinch = opponent.position.x - Math.sign(opponent.position.x || player.position.x || 1) * 5.35;
  const dzClinch = opponent.position.z - Math.sign(opponent.position.z || player.position.z || 1) * 3.85;
  const clinchCornerDistance = Math.sqrt(dxClinch * dxClinch + dzClinch * dzClinch);
...
export function buildControlReadout(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, paused: boolean, device: ControlDevice = 'keyboard', direction: Vec2 = { x: 0, z: 0 }, runHeld = false): ControlReadout {
  ...
  // OPTIMIZATION: Zero-allocation squared magnitude comparison (> 0.0064 equivalent to > 0.08).
  ...
  const nearCorner = Math.abs(player.position.x) > 4.35 && Math.abs(player.position.z) > 2.95;
  const ringside = Math.abs(player.position.x) > 5.82 || Math.abs(player.position.z) > 4.32;
  // OPTIMIZATION: Replace slow Math.hypot with standard Math.sqrt for clinch corner distance.
  const dxClinchCallout = opponent.position.x - Math.sign(opponent.position.x || player.position.x || 1) * 5.35;
  const dzClinchCallout = opponent.position.z - Math.sign(opponent.position.z || player.position.z || 1) * 3.85;
  const clinchCornerDistance = Math.sqrt(dxClinchCallout * dxClinchCallout + dzClinchCallout * dzClinchCallout);
