...
export function buildControlLabels(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, direction: Vec2 = { x: 0, z: 0 }, running = false): Readonly<Record<ControlId, string>> {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for clinch corner distance computation.
  ...
  // OPTIMIZATION: Use zero-allocation squared-magnitude check (> 0.0064 equivalent to > 0.08) to bypass Math.hypot/Math.sqrt.
...
export function buildControlReadout(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, paused: boolean, device: ControlDevice = 'keyboard', direction: Vec2 = { x: 0, z: 0 }, runHeld = false): ControlReadout {
  ...
  const nearCorner = Math.abs(player.position.x) > 4.35 && Math.abs(player.position.z) > 2.95;
  const ringside = Math.abs(player.position.x) > 5.82 || Math.abs(player.position.z) > 4.32;
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for clinch corner distance computation in readout callouts.
  const clinchCornerDx = opponent.position.x - Math.sign(opponent.position.x || player.position.x || 1) * 5.35;
  const clinchCornerDz = opponent.position.z - Math.sign(opponent.position.z || player.position.z || 1) * 3.85;
  const clinchCornerDistance = Math.sqrt(clinchCornerDx * clinchCornerDx + clinchCornerDz * clinchCornerDz);
