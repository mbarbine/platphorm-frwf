...
export function buildControlLabels(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, direction: Vec2 = { x: 0, z: 0 }, running = false): Readonly<Record<ControlId, string>> {
  ...
  // OPTIMIZATION: Replaced Math.hypot with zero-allocation squared distance/magnitude checks to eliminate square root overhead on UI render passes.
  ...
  const effectiveDirection = (direction.x * direction.x + direction.z * direction.z) > .0064
    ? direction
...
export function buildControlReadout(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, paused: boolean, device: ControlDevice = 'keyboard', direction: Vec2 = { x: 0, z: 0 }, runHeld = false): ControlReadout {
  ...
  // OPTIMIZATION: Replacing Math.hypot with zero-allocation squared magnitude check (> 0.0064 equivalent to > 0.08).
  const movementHeld = (direction.x * direction.x + direction.z * direction.z) > .0064;
  ...
  const nearCorner = Math.abs(player.position.x) > 4.35 && Math.abs(player.position.z) > 2.95;
  const ringside = Math.abs(player.position.x) > 5.82 || Math.abs(player.position.z) > 4.32;
  // OPTIMIZATION: Replaced Math.hypot with zero-allocation squared distance check (<= 9.9225 equivalent to <= 3.15).
  const cornerDxReadout = opponent.position.x - Math.sign(opponent.position.x || player.position.x || 1) * 5.35;
  const cornerDzReadout = opponent.position.z - Math.sign(opponent.position.z || player.position.z || 1) * 3.85;
  const clinchCornerDistanceSq = cornerDxReadout * cornerDxReadout + cornerDzReadout * cornerDzReadout;
  ...
  else if (player.state === 'grappling') callout = clinchCornerDistanceSq <= 9.9225
