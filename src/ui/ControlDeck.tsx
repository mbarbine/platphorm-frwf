...
export function buildControlReadout(player: FighterRuntime, opponent: FighterRuntime, speed: number, distance: number, paused: boolean, device: ControlDevice = 'keyboard', direction: Vec2 = { x: 0, z: 0 }, runHeld = false, controlStyle: ControlStyle = 'technical', hasRing = true, grapplePhase: GrappleRuntime['phase'] | null = null): ControlReadout {
  ...
  const calloutCornerDz = opponent.position.z - Math.sign(opponent.position.z || player.position.z || 1) * 3.85;
  const calloutClinchCornerDistanceSq = calloutCornerDx * calloutCornerDx + calloutCornerDz * calloutCornerDz;
  ...
  if (controlStyle === 'arcade' && !paused && ['idle', 'locomotion'].includes(player.state)) {
    ...
    else if (distance <= 3.8 && opponent.state !== 'downed') callout = `${keys.grapple} GRAPPLE → BODY SLAM · ${keys.quick} STRIKE · ${keys.block} GUARD · ${keys.counter} REVERSE`;
    else callout = `${keys.move} CLOSE THE DISTANCE · ${keys.grapple} GRAPPLE · ${actionKey} PIN AFTER THE THROW`;
  }
