...
export function SettingsPanel({ onBack }: { onBack: () => void }) {
  ...
  return <section className="panel panel--settings"><div className="section-heading"><span>ACCESSIBILITY + AUDIO</span><h2>SETTINGS</h2></div>
    <div className="settings-grid">
      ...
      <label className="setting-row setting-row--select"><span>Control deck<b>{settings.controlDeckMode.toUpperCase()}</b></span><select aria-label="Control deck" value={settings.controlDeckMode} onChange={(event) => settings.update({ controlDeckMode: event.target.value as typeof settings.controlDeckMode })}><option value="full">Full · every control</option><option value="compact">Compact · five primary actions</option><option value="prompts">Prompts only · contextual feedback</option><option value="hidden">Hidden · no coaching overlay</option></select></label>
