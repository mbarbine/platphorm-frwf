...
export function SpectatorControls() {
  ...
  return <aside className="spectator-controls" data-testid="spectator-controls" data-camera-mode={cameraMode} data-spectator-target={target}>
    ...
    <div>{MODES.map((mode) => <button key={mode.id} type="button" className={cameraMode === mode.id ? 'active' : ''} aria-pressed={cameraMode === mode.id} aria-label={`Switch to ${mode.label} camera (Press ${mode.key})`} onClick={() => setCameraMode(mode.id)}><kbd>{mode.key}</kbd>{mode.label}</button>)}</div>
    <button type="button" className="spectator-next" aria-label="Spectate next wrestler (Press Tab)" onClick={() => cycleTarget(model)}>NEXT WRESTLER <kbd>TAB</kbd></button>
    ...
    <div aria-live="polite" style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px', overflow: 'hidden', clip: 'rect(0, 0, 0, 0)', border: 0 }}>
      Spectating wrestler: {fighter.name}, {cameraMode.replace('_', ' ')} camera
    </div>
  </aside>;
}
