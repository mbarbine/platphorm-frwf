...
export function SpectatorControls() {
  ...
  const cameraModeLabel = MODES.find((m) => m.id === cameraMode)?.label ?? cameraMode;
  return <aside className="spectator-controls" aria-label="Spectator camera controls" data-testid="spectator-controls" data-camera-mode={cameraMode} data-spectator-target={target}>
    ...
    <div>{MODES.map((mode) => <button key={mode.id} type="button" className={cameraMode === mode.id ? 'active' : ''} aria-pressed={cameraMode === mode.id} aria-label={`${mode.label} camera mode (Shortcut: ${mode.key})`} onClick={() => setCameraMode(mode.id)}><kbd>{mode.key}</kbd>{mode.label}</button>)}</div>
    ...
    <div aria-live="polite" style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px', overflow: 'hidden', clip: 'rect(0, 0, 0, 0)', border: 0 }}>Spectating wrestler: {fighter.name}, {cameraModeLabel} camera</div>
  </aside>;
}
