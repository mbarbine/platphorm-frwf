...
const Meter = ({ label, value, kind, max = 100 }: { label: string; value: number; kind: string; max?: number }) => {
  ...
  return <div className={`meter meter--${kind}`} role="progressbar" aria-label={label} aria-valuenow={roundedValue} aria-valuemin={0} aria-valuemax={max}><div className="meter__label"><span>{label}</span><b>{roundedValue}{max !== 100 ? ` / ${Math.round(max)}` : ''}</b></div><div className="meter__track"><b className="meter__ghost" style={{ width: `${ghostPct}%` }} /><i style={{ width: `${pct}%` }} /></div></div>;
};
