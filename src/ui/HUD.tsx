...
export function HUD({ device, paused }: { device: ControlDevice; paused: boolean }) {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt (~8x faster) for distance calculation on frequent HUD render cycles.
  const distanceDx = model.player.position.x - target.position.x;
  const distanceDz = model.player.position.z - target.position.z;
  const distance = Math.sqrt(distanceDx * distanceDx + distanceDz * distanceDz);
