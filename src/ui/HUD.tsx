...
export function HUD({ device, paused }: { device: ControlDevice; paused: boolean }) {
  ...
  // OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt for rendering and distance calculations.
  const dxDistance = model.player.position.x - target.position.x;
  const dzDistance = model.player.position.z - target.position.z;
  const distance = Math.sqrt(dxDistance * dxDistance + dzDistance * dzDistance);
