...
export function App() {
  ...
  const handleFighterSelectKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') {
      nextIndex = currentIndex - 1 < 0 ? FIGHTERS.length - 1 : currentIndex - 1;
    ...
    if (nextFighter) {
      ...
      // Transfer focus to the selected card programmatically
      setTimeout(() => {
        const btn = document.querySelector(`[data-fighter-select-id="${nextFighter.id}"]`) as HTMLButtonElement | null;
        btn?.focus();
      }, 0);
    }
  };
