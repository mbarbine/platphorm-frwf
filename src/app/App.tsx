...
export function App() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isRosterFocused = activeElement && activeElement.hasAttribute('data-fighter-select-id');
      if (!isRosterFocused) return;

      const currentId = activeElement.getAttribute('data-fighter-select-id') as FighterId;
      const currentIndex = FIGHTERS.findIndex(f => f.id === currentId);
      ...
      if (nextIndex !== currentIndex) {
        ...
        if (nextFighter) {
          ...
          setTimeout(() => {
            const button = document.querySelector(`[data-fighter-select-id="${nextFighter.id}"]`) as HTMLButtonElement | null;
            button?.focus();
          }, 0);
        }
      }
  ...
  }, [screen, setSelected, setBeers, settings]);
