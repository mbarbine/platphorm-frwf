...
export function App() {
  ...
  const [selectionTarget, setSelectionTarget] = useState<'match' | 'world'>('match');
  ...
  const handleRosterKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    e.preventDefault();
    const offset = (e.key === 'ArrowUp' || e.key === 'ArrowLeft') ? -1 : 1;
    const currentIndex = FIGHTERS.findIndex((f) => f.id === selected);
    const nextIndex = (currentIndex + offset + FIGHTERS.length) % FIGHTERS.length;
    const nextCandidate = FIGHTERS[nextIndex];
    if (nextCandidate) {
      setSelected(nextCandidate.id);
      setBeers(0);
      audioEngine.play('menu', settings);
      setTimeout(() => {
        const btn = document.querySelector(`[data-fighter-select-id="${nextCandidate.id}"]`) as HTMLButtonElement | null;
        btn?.focus();
      }, 0);
    }
  };
  ...
  return <main className={`app app--${screen}${toyTest ? ' app--toy-test' : ''}`}>
    ...
    {screen === 'how' && <section className="panel panel--how"><div className="section-heading"><span>CORNER COACH</span><h2>HOW TO PLAY</h2></div><div className="how-grid">
      <article><b>1 · MOVE WITH PURPOSE</b><p>Use WASD to circle your opponent. Hold Shift only when you want to sprint or hit the ropes.</p></article><article><b>2 · STRIKE CLEANLY</b><p>J throws the fast strike. K throws the power strike. Arcade controls keep these attacks consistent while you move. Technical controls add directional variations.</p></article><article><b>3 · WRESTLE UP CLOSE</b><p>Get chest-to-chest and press L for a collar lock. Then use J, K, or L with a direction to choose a takedown, slam, or throw.</p></article><article><b>4 · DEFEND</b><p>Hold I to guard. Tap Space to dodge or reverse during the counter window. Space also helps you recover when down.</p></article><article><b>5 · FOLLOW THE ACTION PROMPT</b><p>F only appears when it matters: pin a downed rival, use a finisher, climb a corner, or move through the ropes.</p></article><article><b>6 · ADVANCED TOOLS</b><p>C jumps, E handles props, and Q taunts. Learn those after the five core controls feel natural.</p></article>
...
export function Results({ result, winnerName, onRematch, onChange, onMenu, onWorld }: { result: NonNullable<ReturnType<typeof useMatchStore.getState>['model']['result']>; winnerName: string; onRematch: () => void; onChange: () => void; onMenu: () => void; onWorld?: () => void }) {
  ...
  return <section className="results-screen"><div className="results-flare" /><div aria-live="polite" style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px', overflow: 'hidden', clip: 'rect(0, 0, 0, 0)', border: 0 }}>Match decision: {winnerName} wins by {result.method}. Hype rating: {result.grade}.</div><span className="results-kicker">{venueFor(useMatchStore.getState().model).name.toUpperCase()} DECISION</span><h2>{winnerName}<small>WINS BY {result.method}</small></h2><div className={`grade grade--${result.grade}`}><span>HYPE RATING</span><b>{result.grade}</b><small>{Math.round(result.hype)} / 100</small></div><div className="results-stats">{rows.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div>{highlights.some(([, moment]) => moment !== null) && <div className="highlight-reel"><strong>MATCH HIGHLIGHT REEL</strong>{highlights.filter((entry) => entry[1] !== null).map(([label, moment]) => <div key={label}><span>{label}</span><b>{moment?.label}</b><small>{moment?.time.toFixed(1)}s · IMPACT {Math.round(moment?.score ?? 0)}</small></div>)}</div>}<div className="circuit-result">{onWorld && <><span>LOCAL CIRCUIT · {circuit.rank.toUpperCase()}</span><b>{circuit.reputation} REPUTATION</b><p>{medals.length ? CIRCUIT_OBJECTIVES.filter(o => medals.includes(o.id)).map(o => `★ ${o.label}`).join(" · ") : "Keep working the circuit. Completed bout recorded."}</p></>}</div><div className="button-row">{onWorld && <button className="button button--hero" onClick={onWorld}>RETURN TO SHOWGROUND</button>}<button className="button button--hero" onClick={onRematch}>INSTANT REMATCH</button><button className="button button--quiet" onClick={onChange}>CHANGE FIGHTER</button><button className="button button--quiet" onClick={onMenu}>MAIN MENU</button></div></section>;
}
