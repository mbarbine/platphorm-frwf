...
export function App() {
  ...
  const finish = useCallback(() => {
    if (worldEncounter) useWorldSession.getState().finish(useMatchStore.getState().model.result?.winner === 'player');
    setScreen('results');
  }, [worldEncounter]);
...
function Results({ result, winnerName, onRematch, onChange, onMenu, onWorld }: { result: NonNullable<ReturnType<typeof useMatchStore.getState>['model']['result']>; winnerName: string; onRematch: () => void; onChange: () => void; onMenu: () => void; onWorld?: () => void }) {
  ...
  return <section className="results-screen"><div className="results-flare" /><span className="results-kicker">OFFICIAL VOLT DOME DECISION</span><h2>{winnerName}<small>WINS BY {result.method}</small></h2><div className={`grade grade--${result.grade}`}><span>HYPE RATING</span><b>{result.grade}</b><small>{Math.round(result.hype)} / 100</small></div><div className="results-stats">{rows.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div>{highlights.some(([, moment]) => moment !== null) && <div className="highlight-reel"><strong>VOLT DOME HIGHLIGHT REEL</strong>{highlights.filter((entry) => entry[1] !== null).map(([label, moment]) => <div key={label}><span>{label}</span><b>{moment?.label}</b><small>{moment?.time.toFixed(1)}s · IMPACT {Math.round(moment?.score ?? 0)}</small></div>)}</div>}<div className="button-row">{onWorld && <button className="button button--hero" onClick={onWorld}>RETURN TO SHOWGROUND</button>}<button className="button button--hero" onClick={onRematch}>INSTANT REMATCH</button><button className="button button--quiet" onClick={onChange}>CHANGE FIGHTER</button><button className="button button--quiet" onClick={onMenu}>MAIN MENU</button></div></section>;
}
