...
export function App() {
  ...
  return <main className={`app app--${screen}${toyTest ? ' app--toy-test' : ''}`}>
    ...
    {screen === 'select' && <section className="select-screen">
      <div className="section-heading"><span>CHOOSE YOUR SIGNAL</span><h2>FIGHTER SELECT</h2></div>
        ...
        <div className="roster" role="list">
          {FIGHTERS.map((candidate, index) => (
            <button
              ...
              onKeyDown={(e) => handleFighterKeyDown(e, index)}
              onClick={() => {
                setSelected(candidate.id);
                setBeers(0);
                setSelectAnnouncement(`Selected ${candidate.name}, ${candidate.archetype}`);
                audioEngine.play('menu', settings);
              }}
            >
      ...
      <div className="button-row">
        <button className="button button--quiet" onClick={() => confirm('main')}>BACK</button>
        <button className="button button--hero" onClick={() => confirm('rules')}>LOCK IN {fighter.name}</button>
      </div>
    </section>}
