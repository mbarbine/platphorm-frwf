...
// OPTIMIZATION: Replacing slow Math.hypot with standard Math.sqrt to prevent CPU-intensive dynamic scaling.
...
const segmentCircleHit = (
  ...
  // OPTIMIZATION: Replacing slow length/Math.sqrt with zero-allocation squared-magnitude check to completely avoid square root extraction on hot collision paths.
...
export const stepOnlineMatch = (match: OnlineMatchState, dt: number): readonly OnlineImpact[] => {
  ...
  for (const actor of match.fighters.values()) {
    ...
    if (target) {
      ...
      // OPTIMIZATION: Replacing slow length check with zero-allocation squared-magnitude comparison to avoid square root extraction on high-frequency distance checks.
    ...
    if (!actor.moveId && !actor.grappleTarget) {
      ...
      // OPTIMIZATION: Replacing slow length check with zero-allocation squared-magnitude comparison to bypass square root computation in locomotion evaluations.
      actor.combatState = actor.guarding ? 'blocking' : (actor.moveX * actor.moveX + actor.moveZ * actor.moveZ) > 0.0025 ? 'locomotion' : 'idle'; // 0.05 * 0.05 = 0.0025
      continue;
    }
