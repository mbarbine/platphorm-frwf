...
const segmentCircleHit = (
  ...
  // OPTIMIZATION: Replacing slow Math.hypot (or standard Math.sqrt) with a zero-allocation squared-magnitude check
  // on hot-path collision/contact evaluations.
...
export const stepOnlineMatch = (match: OnlineMatchState, dt: number): readonly OnlineImpact[] => {
  ...
  for (const actor of match.fighters.values()) {
    ...
    // OPTIMIZATION: Zero-allocation squared-magnitude target proximity check to bypass length calculations in step loops.
    if (target) {
    ...
    if (!actor.moveId && !actor.grappleTarget) {
      ...
      // OPTIMIZATION: Zero-allocation squared-magnitude locomotion checking to bypass length/sqrt on hot paths.
