...
// Since coordinates are bounded, underflow/overflow is not a risk here.
...
// OPTIMIZATION: Completely bypass square root extraction on hot path contact simulation
// by using zero-allocation squared-magnitude comparisons against the squared radius.
...
export const stepOnlineMatch = (match: OnlineMatchState, dt: number): readonly OnlineImpact[] => {
  ...
  for (const actor of match.fighters.values()) {
    ...
    // OPTIMIZATION: Replace length call with a squared-magnitude check to bypass standard square root extraction.
    // Also reuse precomputed delta components dx and dz inside Math.atan2 to avoid duplicate subtraction calculations.
    if (target) {
    ...
    if (!actor.moveId && !actor.grappleTarget) {
      ...
      // OPTIMIZATION: Replacing slow length/Math.sqrt with a zero-allocation squared comparison for locomotion threshold check.
      actor.combatState = actor.guarding ? 'blocking' : (actor.moveX * actor.moveX + actor.moveZ * actor.moveZ) > 0.05 * 0.05 ? 'locomotion' : 'idle';
      continue;
    }
