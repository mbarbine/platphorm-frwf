...
export default function handler(request, response) {
  try {
    ...
    // Prevent unhandled serverless container crashes and stack trace disclosure (CWE-209)
    globalThis.console.error("Unhandled route compliance error:", err)
    return response.status(500).json({
      ok: false,
      error: { code: "internal_server_error", message: "An unexpected error occurred processing route compliance." },
    })
  }
}
