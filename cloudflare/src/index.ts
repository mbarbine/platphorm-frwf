...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (path === '/llms.txt' || path === '/llms-full.txt') return new Response(`# ${gameInfo.name}\n\nVercel and Cloudflare static assets deliver the local game. This Worker implements a separate private-room backend. Consult health for configured storage and auth; client matchmaking is not connected to this Worker.\n\n- [Game](${gameInfo.canonicalUrl})\n- [API](/api/docs)\n- [Health](/api/health)\n- [MCP](/api/mcp)\n\n${gameInfo.online.limitations.join('\n')}\n`, { headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
