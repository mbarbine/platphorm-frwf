...
test('Chaos AI leaves the ring, physically grips a prop, and lands a prop impact', async ({ page }) => {
  ...
  await expect(telemetry).toHaveAttribute('data-prop-bodies', '2');
  await expect.poll(async () => await telemetry.getAttribute('data-opponent-held-prop'), { timeout: 70_000, intervals: [250, 500, 750] }).toMatch(/chair|sign/);
