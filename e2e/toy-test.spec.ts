...
test('UI-free Toy Test preserves body control while removing score pressure', async ({ page }) => {
  ...
  await page.goto('/?toyTest=1'); await page.getByRole('button', { name: 'ENTER RINGFALL' }).click(); await page.getByRole('button', { name: 'PLAY', exact: true }).click();
  ...
  await expect(canvas).toHaveAttribute('data-physics-bodies', /^[1-9]\d*$/, { timeout: 60_000 });
