...
test('UI-free Toy Test preserves body control while removing score pressure', async ({ page }) => {
  ...
  await page.keyboard.down('w'); await expect.poll(async () => Math.hypot(Number(await canvas.getAttribute('data-player-x')) - initialX, Number(await canvas.getAttribute('data-player-z')) - initialZ), { timeout: 3_000, intervals: [100] }).toBeGreaterThan(.75); await page.keyboard.up('w');
